...
public class QuizPlayer : MonoBehaviour
{
    ...
    {
        mobileDemo.onInputFieldSubmit($"Check if the answered response corresponds to the awaited response.Be permissive the answer doesn't need to be exactly the one awaited but if its missing context or words return partial. Answer by true or false or partial. If the anwser is missing some context or words return partial or false. The awaited response is {currentQuiz.inputAnswer}.The answered response is {answerInputField.text}. Respond either true, false or partial. Prioritize true and false. By default return false.");
